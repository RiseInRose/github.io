$(document).ready(function($) {
    "use strict";
	// 修改滚动条状态，在这个位置。配套内容在Header.ejs 的Header & Logo bar 部分，可以添加lightnav-alt即可。
	...
	$(document).ready(function($) {
		...
		// Functionailty constraints for mobile  移动设备的功能限制
		if (!Modernizr.touch) {
		    jQuery(function ($) {
		    ...
		    jQuery(function ($) {
		        // Hero & page-header fade-in effect; // Hero＆page-header淡入效果
		        var divs = $('.headerfade');
		        $(window).on('scroll', function () {
		            var st = $(this).scrollTop();
		            divs.css({
		...
		// autohide navbar on scroll 自动隐藏导航条上滚动
		$("div.navbar-fixed-top").autoHidingNavbar({
			animationDuration: 400,
			hideOffset: 0,
		});

		// faq's floating sidebar (left)  faq的浮动工具条(左)
	    $('#sidebar').affix({
	    	offset: {
	    		top: 500
	    	}
	    ...
	    // Scrollspy for scrollto links in floating faq sidebar ;Scrolllspy用于浮动faq侧边栏中的scrollto链接
		...
		// fade out map cover (contact.html);淡出地图封面（contact.html）
